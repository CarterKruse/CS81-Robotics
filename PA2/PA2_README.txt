...
## Assignment: PA2 - README / Instructions
## Author: Carter Kruse
## Date: April 22, 2023
...
1. Enter into the cloned repository folder, `cd ros-apple-silicon`.
2. Run `docker-compose exec ros bash` (`docker-compose up` has to be running).
3. Run `rosrun stage_ros stageros -d ~/catkin_ws/PA2/2017-02-11-00-31-57.world` to start the simulation.

Open another terminal.

1. Enter into the cloned repository folder, `cd ros-apple-silicon`.
2. Run `docker-compose exec ros bash` (`docker-compose up` has to be running).
3. Enter into the relevant folder, `cd PA2`.
3. Run `python PA2_PD_Controller.py`.
